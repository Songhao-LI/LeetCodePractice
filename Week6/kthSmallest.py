...
class Solution:
    ...
    def inOrderTraversal(self, node: Optional[TreeNode], res):
        if not node:
            return
    ...
    def kthSmallest_recursion(self, root: Optional[TreeNode], k: int) -> int:
        stack = []
        cnt = 0
        while root or stack:
            while root:
